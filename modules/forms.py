...
def set_form_display_mode_logic(body: str, mode: str = "Smooth", lang: str = "en"):
    ...
    try:
        ...
        val = res.get("data", [""])[0]
        if val == "ERR_NOT_A_FORM": return "Error: Target body is not a T-Spline form."
...
def create_form_extrude_logic(sketch: str, dist: float, symmetric: bool = False, lang: str = "en"):
    """Extrudes a sketch profile into a T-Spline form."""
    try:
        res = execute_fusion_script(build_create_form_extrude_script(), {"sketch": sketch, "dist": dist, "symmetric": symmetric})
        val = res.get("data", [""])[0]
...
def create_form_revolve_logic(sketch: str, axis: str = "Z", angle: float = 360.0, lang: str = "en"):
    """Revolves a sketch profile into a T-Spline form."""
    try:
        res = execute_fusion_script(build_create_form_revolve_script(), {"sketch": sketch, "axis": axis, "angle": angle})
        val = res.get("data", [""])[0]
        if val == "ERR_SKETCH": return format_response(lang, "sketch_not_found")
        if isinstance(val, str) and val.startswith("ERR_"): return val
...
def create_form_sweep_logic(profile_sketch: str, path_sketch: str, lang: str = "en"):
    """Sweeps a sketch profile into a T-Spline form along a path."""
    try:
        res = execute_fusion_script(build_create_form_sweep_script(), {"profile_sketch": profile_sketch, "path_sketch": path_sketch})
        val = res.get("data", [""])[0]
...
def create_form_loft_logic(sketch_names: list, lang: str = "en"):
    """Lofts multiple sketch profiles into a T-Spline form."""
    try:
        res = execute_fusion_script(build_create_form_loft_script(), {"sketch_names": sketch_names})
        val = res.get("data", [""])[0]
        if val == "ERR_MIN_PROFILES": return "Error: At least 2 profiles required for form loft."
        if isinstance(val, str) and val.startswith("ERR_"): return val
