...
try:
    ...
    if not target_comp:
        ...
        if m <= 0 or n < 4 or t <= 0:
            ...
            if hd > 0 and (hd / 10.0) >= root_dia:
                returnValue.append(f"ERR_HOLE_TOO_LARGE:{root_dia * 10.0:.3f}")
            else:
                ...
                def offset_point(local_x, local_y):
                    return adsk.core.Point3D.create(local_x, local_y, 0)
                ...
                base_sketch = target_comp.sketches.add(target_comp.xYConstructionPlane)
                ...
                if base_sketch.profiles.count < 1:
                    ...
                    gear_ext = target_comp.features.extrudeFeatures.addSimple(
                    ...
                    tooth_sketch = target_comp.sketches.add(target_comp.xYConstructionPlane)
                    ...
                    root_point1 = offset_point(
                        root_rad * math.cos(math.atan2(start1_local.y, start1_local.x)),
                        root_rad * math.sin(math.atan2(start1_local.y, start1_local.x))
                    )
                    root_point2 = offset_point(
                        root_rad * math.cos(math.atan2(start2_local.y, start2_local.x)),
                        root_rad * math.sin(math.atan2(start2_local.y, start2_local.x))
                    )
                    ...
                    if tooth_sketch.profiles.count < 1:
                        ...
                        tooth_ext = target_comp.features.extrudeFeatures.addSimple(
                        ...
                        if not cyl_face:
                            ...
                            circ_input = target_comp.features.circularPatternFeatures.createInput(input_ents, cyl_face)
                            circ_input.quantity = adsk.core.ValueInput.createByReal(n)
                            pattern_feat = target_comp.features.circularPatternFeatures.add(circ_input)
                            ...
                            combine_input = target_comp.features.combineFeatures.createInput(gear_base, tool_bodies)
                            combine_input.operation = adsk.fusion.FeatureOperations.JoinFeatureOperation
                            target_comp.features.combineFeatures.add(combine_input)

                            if hd > 0:
                                hole_sketch = target_comp.sketches.add(target_comp.xYConstructionPlane)
                                hole_sketch.sketchCurves.sketchCircles.addByCenterRadius(center, hd / 20.0)
                                if hole_sketch.profiles.count > 0:
                                    target_comp.features.extrudeFeatures.addSimple(
