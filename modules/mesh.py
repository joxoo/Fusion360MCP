...
def remesh_body_logic(body: str, density: float = 0.5, lang: str = "en"):
    ...
    try:
        ...
        val = res.get("data", [""])[0]
        if val == "ERR_NOT_A_MESH": return "Error: Target is not a mesh body."
...
def mesh_plane_cut_logic(body: str, plane_name: str = "XY", fill_type: str = "MeshFill", lang: str = "en"):
    ...
    try:
        ...
        val = res.get("data", [""])[0]
        if val == "ERR_INPUT": return "Error: Mesh body or plane not found."
        if isinstance(val, str) and val.startswith("ERR_"): return val
...
def import_mesh_logic(path: str, lang: str = "en"):
    ...
    try:
        ...
        val = res.get("data", [""])[0]
        if val == "ERR_FILE_NOT_FOUND": return f"Error: Mesh file not found at {path}"
        if isinstance(val, str) and val.startswith("ERR_"): return val
